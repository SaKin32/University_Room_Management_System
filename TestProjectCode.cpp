...
void showRoomScheduleWithBooking(string filename, string roomNo, string day) {
    ...
    while (true) {
        ...
        // Wait for Enter to go back to main menu
...
void cancelBookingRoom() {
    ...
    string roomFile, roomNo, dayInput;
    bool back = false;

    while (!back) {
        system("CLS");
        int buildingChoice;
        cout << "\t Which building's booking you want to cancel?\n";
        cout << "\t 1) Building 1\n";
        cout << "\t 2) Building 2\n";
        cout << "\t 3) Building 3\n";
        cout << "\t 4) Back\n\n";
        ...
        if(buildingChoice < 1 || buildingChoice > 3) {
            cout << "\n Invalid building choice! Try again.\n";
            system("pause");
            continue;
        }
        ...
        while(true) { // Room Type Menu
            ...
            // Room file
                        roomFile = (roomType == 1) ? "Classroomsinfo/classroomsB" : "Labroomsinfo/labroomsB";
            roomFile += to_string(buildingChoice) + ".txt";

            // Read available rooms into vector
            ...
            // Ask room number
            while(true) {
                cout << "\nEnter room number to cancel booking or 0 to go back: ";
                cin >> roomNo;

                if(roomNo == "0") 
                  break;

                // Check if roomNo exists in first column of availableRooms
                bool validRoom = false;

                for(auto r: availableRooms) {
                    string firstWord = "";
                ...
                // Ask day
                while(true) {
                    cout << "\nEnter day (Sunday-Saturday) or 0 to go back: ";
                    cin >> dayInput;
                    if(dayInput == "0") break;
                    ...
                    // Check booked slots
                    string key = roomNo + dayInput;
                    if(bookedSlots[key].empty()) {
                        cout << "\nNo booking found for this room on " << dayInput << "!\n";
                        system("pause");
                        break;
                    }

                    // Show booked slots (new page)
                    system("CLS");
                    cout << "\n===== Room " << roomNo << " | " << dayInput << " Booked Slots =====\n\n";
                    for(int s: bookedSlots[key]) {
                        cout << "Slot " << s << "  ";
                    }
                    ...
                    if(bookedSlots[key].count(slot)) {
                        bookedSlots[key].erase(slot);
                        cout << "\n Booking for slot " << slot << " cancelled successfully!\n";
                    } else {
                        cout << "\n Invalid slot number!\n";
                    }
                    system("pause");
                ...
                break; // after day handled, go back to room selection
            } // end room number loop
        } // end room type loop
    } // end building loop
}
...
void viewRoomBookingMenu() {
    ...
    while (true) {
        system("CLS");
        int buildingChoice;
        cout << "\tWhich building's schedule you want to see?\n";
        cout << "\t1) Building 1\n\t2) Building 2\n\t3) Building 3\n\t4) Back\n\n";
        cout << "\tEnter your choice : ";
        cin >> buildingChoice;
        ...
        while (true) { // Room Type Menu
            system("CLS");
            int roomType;
            cout << "\tBuilding " << buildingChoice << ": Which type of room do you want to book?\n";
            cout << "\t1) Classroom\n\t2) Lab Room\n\t3) Back\n\n";
            cout << "\tEnter your choice : ";
            cin >> roomType;

            if (roomType == 3) break; // back to building menu
            if (roomType != 1 && roomType != 2) {
...
while (true) {
    ...
    break; // valid room => move forward
                ...
                while (!validDay) {
                    ...
                    dayInput[0] = toupper(dayInput[0]);
                    for (int i = 1; i < dayInput.size(); i++) dayInput[i] = tolower(dayInput[i]);

                    for (auto d : validDays) if (d == dayInput) validDay = true;
